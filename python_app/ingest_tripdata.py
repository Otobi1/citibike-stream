...
EXPECTED_COLUMNS = [
    ...
    'end_lng',
    'member_casual',
    'month'  # Ensure 'month' column is included and is of DATE type
]
...
def create_tables(engine):
    ...
    try:
        ...
        logging.error(f"Error creating log tables: {e}")
        sys.exit(1)


def create_tripdata_table(engine):
    """
    Creates the tripdata table if it does not exist.
    """
    try:
        with engine.connect() as conn:
            conn.execute(text("""
            ...
            """))
        logging.info("Ensured that tripdata table exists.")
    except Exception as e:
        logging.error(f"Error creating tripdata table: {e}")
        sys.exit(1)
...
def validate_schema(df):
    ...
    Additionally, ensures that the 'month' column is of datetime type.
    """
    missing_columns = set(EXPECTED_COLUMNS) - set(df.columns)
    if missing_columns:
        raise ValueError(f"DataFrame is missing columns: {missing_columns}")
    logging.debug("DataFrame contains all expected columns.")

    # Ensure 'month' is of datetime type
    if not pd.api.types.is_datetime64_any_dtype(df['month']):
        try:
            df['month'] = pd.to_datetime(df['month'])
            logging.info("'month' column converted to datetime.")
        except Exception as e:
            raise ValueError(f"Error converting 'month' column to datetime: {e}")
    else:
        logging.debug("'month' column is already of datetime type.")
...
def copy_dataframe_to_postgres(df, table_name='tripdata'):
    ...
    try:
        ...
        # Generate COPY command without specifying schema
...
def ingest_parquet(engine, file_path):
    ...
    try:
            ...
            while retries < MAX_RETRIES:
                try:
                    ...
                    if retries == MAX_RETRIES:
                        logging.error(
                            f"Batch {batch_num}: Failed to insert records {start} to {end} after {MAX_RETRIES} retries.")
                        ...
                        with engine.connect() as conn:
                            ...
                            """), {'file_name': file_name,
                                   'error_message': f"Batch {batch_num} failed after {MAX_RETRIES} retries."})
        ...
        try:
            ...
            logging.error(f"Failed to move {file_name} to {FAILED_DIR}: {move_error}")
...
def main():
    ...
    parquet_files_to_ingest = [
        parquet_file for parquet_file in parquet_files
        if parquet_file.name not in ingested_files and parquet_file.name not in failed_files
    ]
    if not parquet_files_to_ingest:
        logging.info("No new Parquet files to ingest.")
        sys.exit(0)

    logging.info(f"Found {len(parquet_files_to_ingest)} new Parquet file(s) to ingest.")

    # Ingest Parquet files in parallel
    with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
        future_to_file = {
            executor.submit(ingest_parquet, engine, parquet_file): parquet_file
            for parquet_file in parquet_files_to_ingest
        }
